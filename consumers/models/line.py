...
class Line:
    ...
    def process_message(self, message):
        ...
        topic = message.topic()
        if topic == "nanodegre.kafka.connect.transformed-stations":
            try:
                value = json.loads(message.value())
                self._handle_station(value)
            except Exception as e:
                logger.fatal("bad station? %s, %s", value, e)
        elif "nanodegre.kafka.chicago_cta.station_" in topic:  # Set the conditional to the arrival topic
            self._handle_arrival(message)
        elif "TURNSTILE_SUMMARY" == topic:  # Set the conditional to the KSQL Turnstile Summary Topic
